...
public class InterfazBotones {
	...
	public void añadirQuitarIncompatibilidad(EquipoTrabajo equip,InterfazChoices jChoice) {
		...
		public void actionPerformed(ActionEvent e) {
			equip.addIncompatibilidad(equip.getemploye(jChoice.getEmpleado1()),equip.getemploye(jChoice.getEmpleado2()));
			//System.out.println(equip);
		}
	});
	buttonquitarIncompatibilidad.addActionListener(new ActionListener() {
		public void actionPerformed(ActionEvent e) {
			equip.quitarIncompatibilidad(equip.getemploye(jChoice.getEmpleado1()),equip.getemploye(jChoice.getEmpleado2()));
		}
	});
	}
	public void añadirQuitarEmpleado(InterfazTextoaddEmpleados jText, InterfazChoices jChoice,EquipoTrabajo equip,InterfazListaEmpleados jList) {
					...
					if(!jText.getNombre().isEmpty()) {
						if(!jText.getApellido().isEmpty()) {
							if(!equip.perteneceEmpleado(jText.getDNI())) {
								equip.nuevoEmpleado(jChoice.getRol(), jText.getDNI(), jText.getNombre(),jText.getApellido());
								jText.actualizar();
	...
	public void accionFuerzaBruta(FuerzaBruta fuerzaBruta,EquipoTrabajo equip,InterfazTexto jText,InterfazListaEmpleados jList) {
		button.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				
				Thread HiloFuerzaBruta= new Thread(fuerzaBruta);
				HiloFuerzaBruta.start(); //Inicio del hilo paralelo.
				fuerzaBruta.crearGrupo(equip, jText.getValue(1), jText.getValue(2),jText.getValue(3), jText.getValue(4));
